  ...
  return (
    <div style={{ display: "grid", gap: 8 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <label style={{ color: "#94a3b8", fontSize: 12, fontWeight: 800, textTransform: "uppercase", letterSpacing: ".07em" }}>{label}</label>
        {altUnit && (
          <button type="button" onClick={() => setUseAlt(u => !u)} style={{ background: "rgba(163,230,53,.12)", border: "1px solid rgba(163,230,53,.3)", borderRadius: 999, padding: "3px 10px", fontSize: 11, fontWeight: 800, color: "#a3e635", cursor: "pointer" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
        <input type="range" min={displayMin} max={displayMax} step={step} value={displayVal} onChange={handleSlider}
          style={{ flex: 1, height: 6, borderRadius: 999, outline: "none", cursor: "pointer", appearance: "none", WebkitAppearance: "none", background: `linear-gradient(to right,#a3e635 ${pct}%,rgba(148,163,184,.18) ${pct}%)` }} />
...
function LaunchBanner() {
  ...
  return (
    <div style={{ marginTop: 18, marginBottom: 18, borderRadius: 16, padding: "14px 18px", background: "linear-gradient(135deg,rgba(163,230,53,.12),rgba(56,189,248,.08))", border: "1px solid rgba(163,230,53,.25)", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12 }}>
      <div>
        <div style={{ fontSize: 13, fontWeight: 800, color: "#a3e635", marginBottom: 2 }}>Launch offer - 25% off</div>
        <div style={{ fontSize: 11, color: "#64748b" }}>Launch offer may end or change at any time.</div>
      </div>
      <div style={{ fontVariantNumeric: "tabular-nums", fontSize: 22, fontWeight: 900, color: "#f8fafc", letterSpacing: 1 }}>{mins}:{secs}</div>
    </div>
  );
}
    ...
    <div style={{ ...S.card, overflow: "hidden", position: "relative" }}>
      ...
      <div style={{ height: 140, position: "relative" }}>
        ...
        <div style={{ position: "absolute", left: 14, bottom: 14 }}>
          <Pill color="#a3e635">{recipe.category}</Pill>
        ...
        {open && (
          <div>
            <div style={{ marginBottom: 12 }}>
              <div style={{ color: "#a3e635", fontSize: 11, fontWeight: 800, textTransform: "uppercase", marginBottom: 6 }}>Ingredients</div>
            ...
            <div>
              <div style={{ color: "#a3e635", fontSize: 11, fontWeight: 800, textTransform: "uppercase", marginBottom: 6 }}>Instructions</div>
              {recipe.steps.map((step, i) => (
                <div key={i} style={{ color: "#cbd5e1", fontSize: 13, padding: "6px 0", borderTop: "1px solid rgba(148,163,184,.08)", display: "flex", gap: 10 }}>
                  <span style={{ color: "#a3e635", fontWeight: 900, flexShrink: 0, minWidth: 18 }}>{i + 1}.</span>{step}
                </div>
              ))}
            </div>
          </div>
...
export default function App() {
  ...
  function choosePlan(id) {
    ...
    if (tier === "free") { setAccess("free"); setScreen("onboarding"); }
    else setScreen("unlock");
  }

  function unlockWithCode() {
    // DEMO UNLOCK ONLY - unlocks Elite for any valid owner code regardless of plan purchased.
    // Production must verify payment tier via backend/webhook before granting access.
    if (unlockCode.trim() === OWNER_CODE) { setAccess("elite"); setSelTier("elite"); setUError(""); setScreen("onboarding"); }
    else setUError("Invalid unlock code. If you paid, contact " + SUPPORT_EMAIL + " with your PayPal receipt.");
  }
    ...
    <div style={S.page}>
      <div style={{ ...S.wrap, padding: "28px 0 70px" }}>
        <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 40, flexWrap: "wrap", gap: 10 }}>
          ...
          <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
            ...
            {user ? (
              ...
              <button onClick={() => setAuthModal("login")} style={{ ...S.sec, padding: "8px 14px", borderRadius: 999, fontSize: 13 }}>Sign in</button>
            )}
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(210px,1fr))", gap: 14 }}>
          {PLANS.map(p => (
            <div key={p.id} style={{ ...S.card, padding: 20, position: "relative", border: p.recommended ? "1px solid rgba(163,230,53,.5)" : p.premium ? "1px solid rgba(192,132,252,.5)" : S.card.border, background: p.recommended ? "linear-gradient(180deg,rgba(163,230,53,.1),rgba(15,23,42,.8))" : p.premium ? "linear-gradient(180deg,rgba(192,132,252,.1),rgba(15,23,42,.8))" : S.card.background }}>
              {p.recommended && <div style={{ position: "absolute", top: -10, left: "50%", transform: "translateX(-50%)", background: "#a3e635", color: "#020617", fontSize: 11, fontWeight: 900, padding: "4px 14px", borderRadius: 999, whiteSpace: "nowrap" }}>MOST POPULAR</div>}
              <Pill>{p.badge}</Pill>
              ...
              {p.features.map(f => <div key={f} style={{ color: "#cbd5e1", fontSize: 13, marginBottom: 8, display: "flex", gap: 8, alignItems: "flex-start" }}><span style={{ color: "#a3e635", flexShrink: 0 }}>✓</span>{f}</div>)}
      ...
      <div style={{ ...S.wrap, maxWidth: 540, padding: "28px 0 70px" }}>
        ...
        <div style={{ ...S.card, padding: 28, marginTop: 18 }}>
          ...
          <div style={{ background: "rgba(2,6,23,.5)", borderRadius: 14, padding: "14px 16px", marginBottom: 22 }}>
            {selPlan.features.map(f => <div key={f} style={{ color: "#cbd5e1", fontSize: 14, marginBottom: 8, display: "flex", gap: 8 }}><span style={{ color: "#a3e635" }}>✓</span>{f}</div>)}
          </div>
        ...
        <div style={{ ...S.card, padding: 24, display: "grid", gap: 20 }}>
          ...
          <div style={{ display: "flex", gap: 12, alignItems: "flex-start", padding: 16, background: "rgba(2,6,23,.5)", borderRadius: 12, border: termsError ? "1px solid #f87171" : "1px solid rgba(148,163,184,.12)" }}>
            ...
            <label htmlFor="terms-agree" style={{ color: "#94a3b8", fontSize: 13, lineHeight: 1.6, cursor: "pointer" }}>
              ...
              <span style={{ color: "#a3e635", textDecoration: "underline", cursor: "pointer" }} onClick={() => setLegalModal("terms")}>Terms of Service</span>{" "}and{" "}
    ...
    return (
      <div style={S.page}>
        <div style={{ ...S.wrap, padding: "24px 0 80px" }}>
          ...
          {saveMsg && <div style={{ borderRadius: 12, padding: "10px 16px", background: saveMsg.includes("saved") ? "rgba(163,230,53,.1)" : "rgba(248,113,113,.1)", border: "1px solid " + (saveMsg.includes("saved") ? "rgba(163,230,53,.3)" : "rgba(248,113,113,.3)"), marginBottom: 14, fontSize: 13, color: saveMsg.includes("saved") ? "#bbf7d0" : "#fca5a5" }}>{saveMsg}</div>}
          <LaunchBanner />
          ...
          {planReady && (
            <div style={{ borderRadius: 14, padding: "14px 18px", background: "rgba(163,230,53,.08)", border: "1px solid rgba(163,230,53,.25)", marginBottom: 16, fontSize: 14, color: "#bbf7d0", display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 20, flexWrap: "wrap" }}>
            {tabs.map(t => (
              <button key={t} onClick={() => setActiveTab(t)} style={{ flex: "1 1 80px", minWidth: 80, background: activeTab === t ? "#a3e635" : "rgba(15,23,42,.72)", color: activeTab === t ? "#020617" : "#cbd5e1", border: "1px solid " + (activeTab === t ? "#a3e635" : "rgba(148,163,184,.16)"), borderRadius: 14, padding: "12px 8px", fontWeight: 800, cursor: "pointer", fontSize: 14, textTransform: "capitalize" }}>
          ...
          {activeTab === "meals" && (<>
            <div style={{ display: "flex", gap: 8, overflowX: "auto", paddingBottom: 10, marginBottom: 20, WebkitOverflowScrolling: "touch" }}>
              {plan.days.map((d, i) => (
                <button key={d.day} onClick={() => setOpenDay(i)} style={{ flexShrink: 0, background: openDay === i ? "#a3e635" : "rgba(15,23,42,.72)", color: openDay === i ? "#020617" : "#cbd5e1", border: "1px solid " + (openDay === i ? "#a3e635" : "rgba(148,163,184,.16)"), borderRadius: 999, padding: "10px 16px", fontWeight: 800, cursor: "pointer", whiteSpace: "nowrap", fontSize: 14 }}>
          ...
          {activeTab === "recipes" && (
            <div>
              {!hasRecipes(plan.tier) && (
                <div style={{ ...S.card, padding: 22, marginBottom: 20, background: "linear-gradient(135deg,rgba(163,230,53,.08),rgba(15,23,42,.8))", border: "1px solid rgba(163,230,53,.3)" }}>
